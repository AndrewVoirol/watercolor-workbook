...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  if (surf_depth > 0.004) {
    ...
    let curl_vortex = vec2<f32>(-grad_c.y, grad_c.x) * 0.50;
    
    let raw_m_force = -(grad_c + curl_vortex) * (uniforms.marangoni_flow_rate * 0.60) * dt * 10.0;
    let m_mag = length(raw_m_force);
    let marangoni_force = select(vec2<f32>(0.0), (raw_m_force / max(m_mag, 0.0001)) * min(m_mag, 1.2), m_mag > 0.001);
  ...
  let tooth_drag = uniforms.paper_drag * uniforms.paper_roughness * (0.6 + 0.6 / (1.0 + clearance * 20.0));
  
  let wet_slip = clamp(1.0 - (advected_water.g * 0.45 + surf_depth * 0.75), 0.18, 1.0);
  let effective_drag = (tooth_drag * wet_slip * (1.0 + fiber_density * 0.4)) + uniforms.viscosity * 0.6;
  let drag_factor = clamp(1.0 - effective_drag * dt * 2.2, 0.0, 1.0);
